...
def iou(mask_a, mask_b, class_id):
    return np.nan_to_num(((mask_a == class_id) & (mask_b == class_id)).sum(axis=(1, 2)) / (
                (mask_a == class_id) | (mask_b == class_id)).sum(axis=(1, 2)), 0, 0, 0)
...
def count_pixels(label_file, class_id=None):
    ...
    if class_id:
        return float(np.bincount(np_img.astype(int).flatten(), minlength=2)[class_id]) / TOTAL_PIXELS
    else:
        return [float(p) / TOTAL_PIXELS for p in np.bincount(np_img.astype(int).flatten(), minlength=2)]
