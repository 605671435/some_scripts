...
def slice_ct(ct, path, id):
    ct_array = sitk.GetArrayFromImage(ct)
    for i in range(ct_array.shape[0]):
        slice = ct_array[i]
        save_path = osp.join(path, id + '_' + str(i) + '.jpg')
...
class LITS_preprocess:
    ...
    def fix_data(self):
        ...
        all_list = os.listdir(self.raw_root_path)
        file_list = []
        for file in all_list:
            if file.find('volume') == 0:
                file_list.append(file)
        Numbers = len(file_list)
        Training_Numbers = int(Numbers * 0.2)
        print('Total numbers of samples is :', Numbers)
        progress_bar = ProgressBar(Numbers)
        for ct_file, i in zip(file_list, range(Numbers)):
            ...
            seg_path = os.path.join(self.raw_root_path, ct_file.replace('volume', 'segmentation'))
            new_ct, new_seg = self.process(ct_path, seg_path, classes=self.classes)
            if new_ct != None and new_seg != None:
                new_name = ct_file.replace('volume-', '').replace('.nii', '.nii.gz')
                if i < Training_Numbers:
                    slice_ct(new_ct, os.path.join(self.fixed_path, 'images', 'training'), ct_file.split('.')[0])
                    slice_ct(new_seg, os.path.join(self.fixed_path, 'annotations', 'training'), ct_file.split('.')[0])
                    # sitk.WriteImage(new_ct, os.path.join(self.fixed_path, 'images', 'training', new_name))
                    # sitk.WriteImage(new_seg, os.path.join(self.fixed_path, 'annotations', 'training', new_name))
                else:
                    slice_ct(new_ct, os.path.join(self.fixed_path, 'images', 'validation'), ct_file.split('.')[0])
                    slice_ct(new_seg, os.path.join(self.fixed_path, 'annotations', 'validation'), ct_file.split('.')[0])
                    # sitk.WriteImage(new_ct, os.path.join(self.fixed_path, 'images', 'validation', new_name))
                    # sitk.WriteImage(new_seg, os.path.join(self.fixed_path, 'annotations', 'validation', new_name))
            progress_bar.update()
    ...
    def process(self, ct_path, seg_path, classes=None):
        ...
        print("\nOri shape:", ct_array.shape, seg_array.shape)
        if classes == 2:
            # 将金标准中肝脏和肝肿瘤的标签融合为一个
...
if __name__ == '__main__':
    raw_dataset_path = './data/lits17/ori_data'
    fixed_dataset_path = './data/lits17/fixed_data_imgs_512x512'
